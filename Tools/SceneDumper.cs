...
using BepInExLoader = DiscoTranslatorFinalCut.PluginLoader;

namespace DiscoTranslatorFinalCut.Tools
{
            ...
            foreach (var c in obj.GetComponents<Component>())
            {
                ...
                if (Main.optionToggle)
                {
                    ...
                    foreach (var p in props)
                    {
                        ...
                        {
                            sw.WriteLine(pad3 + "@" + p.Name + "<" + p.PropertyType.Name + "> = null => "+e.Message);
